...
- Packaging note for that activation: install a session D-Bus service file for `com.hplip.StatusService` whose activator is `hp-systray --force-startup`, for example
  ```
  [D-BUS Service]
  Name=com.hplip.StatusService
  Exec=/usr/bin/hp-systray --force-startup
  SystemdService=scanbus-hplip-status.service
  ```
  `SystemdService=` is optional and only belongs there if packaging wants user-systemd to supervise the activator. The executable must stay `hp-systray --force-startup`, not `hpssd.py`: HPLIP's own startup path goes through `/usr/share/hplip/base/device.py` into `hp-systray`, and `hp-systray` is what forks `hpssd` plus its helper pipes before the child claims `com.hplip.StatusService`.
